...
| 业务员检查 | `/sales/demands` | 按日浏览；填客户代码/名时默认跨日搜索（忽略日期）；pending 可改，done 只读；均可删除 |
...
- 需求一单一个 `customer_code`（必填）+ 可选 `customer_name` + 一种材料；新提交/编辑均按「一材料一段」；编辑保留材料拼板状态，不能丢掉已 done 的材料
...
`{前缀}-{YYYYMMDD}-板{编号}-{材料名}-{uid8}.eps`，例如 `卡纸路径-20260805-板1-黑卡纸-a1b2c3d4.eps`。
